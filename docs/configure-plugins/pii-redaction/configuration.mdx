...
managed call supplies a recognized provider codec.
...
- LLMs use the active built-in codec for that managed call. Prefer normalized Relay paths such as:
...
recommended contract for new configuration. If normalized paths are configured
and Relay has neither an active recognized codec nor a configured compatibility
fallback, the LLM event payload and its annotation are omitted rather than
emitting unsanitized content.
