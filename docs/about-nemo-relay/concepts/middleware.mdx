...
LLM sanitize guardrails have two compatible callback forms. Existing callbacks
continue to receive only the payload and must return a sanitized payload. New
**contextual** callbacks receive the payload first, then per-call codec
information:
...
Contextual and legacy callbacks use the same registry and therefore preserve
the normal priority and name tie-break ordering. Use a contextual callback when
the sanitization rule depends on the active provider codec; retain the legacy
form for provider-independent sanitization.
