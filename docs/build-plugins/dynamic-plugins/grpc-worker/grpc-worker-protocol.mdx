...
For contextual LLM sanitizer registrations, set `Registration.contextual` to
`true`. The corresponding `LlmInvocation` includes `has_active_codec` and an
optional `codec_name`. The worker callback receives that context alongside the
request or response JSON. It returns a normal JSON result to continue, or an
empty result to omit the LLM observability payload and annotation. The legacy
LLM sanitizer registrations and payload forms remain valid.
