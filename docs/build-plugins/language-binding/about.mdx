...
LLM sanitize guardrails also have a contextual form for provider-aware
observability policies. A contextual callback receives the request or response
first, followed by `has_active_codec` and an optional canonical `codec_name`.
It may omit the LLM event payload by returning no value. Native and
gRPC worker plugins receive the same JSON-compatible context; legacy sanitizer
callbacks remain supported unchanged.
